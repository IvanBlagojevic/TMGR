...
public class PowerUserRoleFactory extends AbstractRoleFactory {

    private static final String SYSTEM_POWER_USER = RoleNameEnum.SYSTEM_POWER_USER.name();
	...
	Role system_power_user = new Role();
	system_power_user.setRoleId(SYSTEM_POWER_USER.toLowerCase());
	system_power_user.setRoleType(RoleTypeEnum.SYSTEM);
	system_power_user.setGeneric(Boolean.FALSE);
	system_power_user.setPolicies(createSystemPolicies());
	return newHashSet(system_power_user);
    }
	...
	Policy policy_1 = createPolicy(RoleTypeEnum.SYSTEM, "Project", "POLICY_FOUNDATION_PROJECT_ADD");
	Policy policy_2 = createPolicy(RoleTypeEnum.SYSTEM, "Project", "POLICY_FOUNDATION_PROJECT_EDIT");
	Policy policy_3 = createPolicy(RoleTypeEnum.SYSTEM, "Project", "POLICY_FOUNDATION_PROJECT_ENABLEDISABLE");
	Policy policy_4 = createPolicy(RoleTypeEnum.SYSTEM, "Project", "POLICY_FOUNDATION_PROJECT_VIEW");
	Policy policy_5 = createPolicy(RoleTypeEnum.SYSTEM, "Project", "POLICY_TM_PROJECT_SEND_CONNECTION");

	Policy policy_6 = createPolicy(RoleTypeEnum.SYSTEM, "Term", "POLICY_TM_MENU_TERMS");
	Policy policy_7 = createPolicy(RoleTypeEnum.SYSTEM, "Term", "POLICY_TM_VIEW_TRANSLATOR_INBOX");

	Policy policy_8 = createPolicy(RoleTypeEnum.SYSTEM, "User", "POLICY_FOUNDATION_USERPROFILE_EDIT");

	return newHashSet(policy_1, policy_2, policy_3, policy_4, policy_5, policy_6, policy_7, policy_8);
    }

    private String getProjectRoleId() {
	final int index = SYSTEM_POWER_USER.indexOf("_");
	String roleId = SYSTEM_POWER_USER.substring(index);
