...
public class ProjectLanguageDetailServiceImpl implements ProjectLanguageDetailService {
	...
	// exclude dummy terms
	long termInSubmission = pldForClone.getTermInSubmissionCount() - dummyTermsCount;
	...
	projectLanguageDetail.setTermCount(pldForClone.getTermCount() - dummyTermsCount);
	...
	if (Objects.nonNull(pld) && pld.isDisabled()) {
	    Set<ProjectLanguageUserDetail> userDetails = pld.getUserDetails();
	    userDetails.forEach(userDetail -> getProjectLanguageUserDetailDAO().delete(userDetail));
